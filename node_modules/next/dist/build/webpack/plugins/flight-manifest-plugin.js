...
class ClientReferenceManifestPlugin {
    ...
    createAsset(assets, compilation, context) {
        ...
        compilation.chunkGroups.forEach((chunkGroup)=>{
            ...
            const groupName = /\/page(\.[^/]+)?$/.test(entryName) ? entryName.replace(/\/page(\.[^/]+)?$/, "/page") : entryName.slice(0, entryName.lastIndexOf("/"));
            ...
            if (entryName.includes("/@")) {
                ...
                const groupNameWithoutNamedSegments = entryNameWithoutNamedSegments.slice(0, entryNameWithoutNamedSegments.lastIndexOf("/"));
        ...
        for (const [groupName] of manifestsPerGroup){
            if (groupName.endsWith("/page") || groupName === "app/not-found") {
                ...
                assets["server/" + pagePath + "_" + _constants.CLIENT_REFERENCE_MANIFEST + ".js"] = new _webpack.sources.RawSource(`globalThis.__RSC_MANIFEST=(globalThis.__RSC_MANIFEST||{});globalThis.__RSC_MANIFEST[${JSON.stringify(pagePath.slice("app".length))}]=${JSON.stringify(json)}`);
                if (pagePath === "app/not-found") {
                    // Create a separate special manifest for the root not-found page.
                    assets["server/" + "app/_not-found" + "_" + _constants.CLIENT_REFERENCE_MANIFEST + ".js"] = new _webpack.sources.RawSource(`globalThis.__RSC_MANIFEST=(globalThis.__RSC_MANIFEST||{});globalThis.__RSC_MANIFEST[${JSON.stringify("/_not-found")}]=${JSON.stringify(json)}`);
                }
            }
        }
